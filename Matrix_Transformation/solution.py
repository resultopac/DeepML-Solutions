...
    """
    Perform the change-of-basis transform T⁻¹ A S and round to 3 decimals using 
    PyTorch.
...
    T_Invers = torch.linalg.inv(T_t)
    if torch.linalg.det(T_t) == 0 or torch.linalg.det(S_t) == 0 :
        return torch.tensor([-1])
    pre_ = torch.matmul(T_Invers, A_t)
    return torch.matmul(pre_, S_t)
